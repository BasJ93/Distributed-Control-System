...
static DEFINE_MUTEX(hello_device_mutex);
...
static ssize_t hello_read(struct file* filp, char __user *buffer, size_t lenght, loff_t* offset)
{
	struct PID_data *PID;
	ssize_t retval;
	ssize_t copied = 0;
...
static ssize_t hello_write(struct file* filp, const char __user *buffer, size_t lenght, loff_t* offset)
{
	struct PID_data *PID;
	ssize_t retval;
	int converted_value;
...
static int hello_open(struct inode* inode, struct file* filp)
{
	...
	if(!mutex_trylock(&hello_device_mutex))
	{
...
static int hello_release(struct inode* inode, struct file* filp)
{
	...
	mutex_unlock(&hello_device_mutex);
	return 0;
}
...
static int hello_init(void)
{
	...
	hello_major = register_chrdev(0, DEVICE_NAME, &hello_fops);
...
/*
	...
	retval = device_create_file(hello_device, &dev_attr_I);
	if(retval < 0)
		printk(KERN_WARNING "Failed to create /sys endpoint - continuing\n");
