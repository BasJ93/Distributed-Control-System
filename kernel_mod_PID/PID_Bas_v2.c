...
static const int N_PID_MINORS = 32;
//JNSN: Als ik bovenstaande gebruik in plaats van een define krijg ik volgende compiler error:
//error: variably modified 'minors' at file scope
// static DECLARE_BITMAP(minors, N_PID_MINORS);
//                       ^
//include/linux/types.h:10:16: note: in definition of macro 'DECLARE_BITMAP'
...
static struct PID_data {   // HKMS: waarom niet static? JNSN: Als ik dit static maak krijg ik een compiler error.
...
static ssize_t sys_set_node(struct device* dev, struct device_attribute* attr, const char* buffer, size_t lenght)
{
	...
	list_for_each_entry(PID, &device_list, device_entry) {
		...
		if(PID->devt == dev->devt) {
			...
			// HKMS: else printk("dit zou niet mogen gebeuren!!!") en return!, ofwel: voorkomen dat een uitbreiding later problemen oplevert!
		}
	}
